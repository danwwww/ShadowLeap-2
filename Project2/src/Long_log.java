...
public class Long_log extends Sprite{
	private boolean moveRight;
	private static final float Speed=0.1f;
	private static final float tileSizeW=228;
	...
	public Long_log(float x, float y,boolean moveR) throws SlickException {
		this.x=x;
		this.y=y;
		this.setImage(new Image(LONGLOG_PATH));
		this.setbBox(new BoundingBox(x,y,tileSizeW,tileSizeH,"Long_log"));
	...
	public void update(Extra_life extra,Frog player,int delta) {
		...
		{	 if(this.isMoveRight()==true) {
				player.move(delta*Long_log.Speed, 0);	
			}
			else {
				player.move(delta*Long_log.Speed*(-1), 0);
			}
		}
		
		if(this.contactSprite(extra)==true) 
		{	 if(this.isMoveRight()==true) {
			extra.move2(delta*Long_log.Speed);	
			}
			else {
				extra.move2(delta*Long_log.Speed*(-1));
			}
		}
